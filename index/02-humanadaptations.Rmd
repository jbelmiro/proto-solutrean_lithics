...
This climatic variability is felt not only at a chronological level but also at a geographic level. Land conditions (achieved by the analysis of pollen samples within the cores) seem to have been different in northern Iberia and southwestern Iberia as shown through the recovered cores in the southwestern area off Portugal: SU81-18 [@turon2003]; and to the northwestern Iberia, off Galicia: MD99-2331 and MD03-2697 [@naughton2007]. In conjunction with other climate-proxy data, such as the Greenland ice cores, palynological studies, charcoal studies and faunal analysis, these records have allowed for the characterization of the paleo-climatic conditions of western and southern Iberia during the Pleistocene [@cascalheiraandbicho2013; @sanchez-goni2010].
...
```{r su8118, fig.cap="Diagram showing the main pollen and spore taxa and dinoflagellate cyst specimens per depth (Turon et al. 2003).",fig.align='center', out.width = '80%', echo=FALSE, fig.pos="H"}
...
```{r protomap, fig.cap="Location of Proto-Solutrean sites in Portugal, including the two sites analysed in this study.",fig.align='center', out.width = '80%', echo=FALSE, fig.pos="H"}
...
```{r twophase, fig.cap="Two-phase model for Gravettian to Proto-Solutrean transition, with site and archaeological level correlated with each phase. Adapted from Zilhão (1997). Model dates calibrated with curve IntCal13, using OxCal 4.1.7 (online).", fig.align = 'center', out.width = '70%', echo=FALSE, fig.pos="H"}
knitr::include_graphics("figure/Two-phasemodel.png")
```

```{r threephase, fig.cap="Three-phase model for Gravettian to Proto-Solutrean transition, with site and archaeological level correlated with each phase. Adapted from Zilhão (1997). Model dates calibrated with curve IntCal13, using OxCal 4.1.7 (online).", fig.align = 'center', out.width = '70%', echo=FALSE, fig.pos="H"}
knitr::include_graphics("figure/Three-phasemodel.png")
```

\  

Alternatively, in the Two-phase model, the Aurignacian V, is understood as a functional facies, related to specialized occupations and production activities within the technocomplex, through the observation of the coexistence of prismatic core exploitation and carinated core exploitation. The fact that both showed extraction of blades and bladelets, the latter exploitation was interpreted not an independent system as thought at Laugerie Haute [@zilhao1997; @zilhaoetal1999]. In fact, Zilhão [-@zilhao1994; -@zilhao1997] has questioned the stratigraphic reality of the Aurignacian V from Laugerie-Haute, interpreting it only as a typology separation due to poor stratigraphic integrity.
