...
Testing this apparent division (artifacts wider than 12.5 mm and those with less than 12.5 in width) against all other relevant morphological variables revealed, however, no significant difference between the two groups (Fisher's exact test for: *Platform Type* p-value = `r round(plat$p.value, 2)`, *Profile* p-value = `r count(prof$p.value, 2)`, *Cross section* p-value = `r round(cross$p.value, 2)`, *Edge Shape* p-value = `r round(shape$p.value, 2)`). These results, together with the fairly linear positive correlation showed by the chert tendecy line displayed on Figure \@ref(fig:elongdisp2), seem enough to do the technological description of all elongated products as a single category.
...
```{r fishertestelongLP, echo = FALSE}
...
platLP1 <- fisher.test(table(elong_groups_LP1$PlatformType, elong_groups_LP1$elong_group))
profLP1 <- fisher.test(table(elong_groups_LP1$Profile, elong_groups_LP1$elong_group))
crossLP1 <- fisher.test(table(elong_groups_LP1$CrossSection, elong_groups_LP1$elong_group))
shapeLP1 <- fisher.test(table(elong_groups_LP1$BlankShape, elong_groups_LP1$elong_group))
...
platLP2 <- fisher.test(table(elong_groups_LP2$PlatformType, elong_groups_LP2$elong_group))
profLP2 <- fisher.test(table(elong_groups_LP2$Profile, elong_groups_LP2$elong_group))
crossLP2 <- fisher.test(table(elong_groups_LP2$CrossSection, elong_groups_LP2$elong_group))
shapeLP2 <- fisher.test(table(elong_groups_LP2$BlankShape, elong_groups_LP2$elong_group))
