...
# Read db
context <- read_csv("index/data/context.csv") #Field DB
xyz <- read_csv("index/data/xyz.csv") #Field DB
lithics <- read_csv("index/data/Basedados_VB.csv") #Analysis DB
...
lithicslp <- read_csv("index/data/Basedados_LP.csv")
xyzlp <- read_csv("index/data/xyzlp.csv")
cols_to_concat <- c("UNIT", "ID")
...
Core flattening (Figure \@ref(fig:coreboxplotVB)) (calculated by dividing maximum width by thickness) reveals that greywacke and other raw materials show higher values, while quartz and chert cores have similar values for both phases (Chert t-test p-value = `r ttest_flat_cores_chert$p.value`, Quartz t-test p-value = `r ttest_flat_cores_quartz$p.value`), with flattening median values of around 1.5, centered in between the second and third quartiles of the distribution.


```{r corebloxplotVB, fig.cap="Vale Boi. Boxplots of core elongation and flattening by raw material and phase.", echo=FALSE}
...
```{r coretypeLP, echo = FALSE, fig.cap="Vale Boi. Interaction of core type with type of extracted products by raw material and phase."}
...
```{r corebloxplotLP, fig.cap="Lapa do Picareiro. Boxplots of core elongation and flattening by raw material and phase.", echo=FALSE}
coreB_LP <- datasetlp %>% 
  filter(Class=="Core") %>% 
  mutate(CoreElongation=Length/MaxWidth) %>% 
  mutate(CoreFlattening=MaxWidth/Thickness) %>% 
...
```{r flakeboxplot, fig.cap="Vale Boi. Boxplots of flake elongation and flattening by raw material and phase.", echo=FALSE}
...
knitr::kable(elong_test_AT2, longtable= TRUE, booktabs = TRUE, caption = "Vale Boi - Upper5/4E. Results of Fisher exact tests to evaluate whether the distribution of technological variables differed across metric groups (blades and bladelets) of chert elongated artifacts.") %>% 
...
elong_test_LP2 <- tabmulti(elong_groups_LP1, "elong_group", c("CrossSection", "BlankShape", 
                                                           "Profile", "BlankTip",
                                                         "PlatformType", 
...
knitr::kable(elong_test_LP2, longtable= TRUE, booktabs = TRUE, caption = "Lapa do Picareiro - U/Lower T. Results of Fisher exact tests to evaluate whether the distribution of technological variables differed across metric groups (blades and bladelets) of chert elongated artifacts.") %>% 
  row_spec(0, bold = T, align = "c") %>% 
  kable_styling(position = "center")
...
#### Vale Boi
As seen before, retouched pieces make up a small percentage of the assemblage, with 167 identified pieces, from which 55 can be inserted in the Lower 5 group (fig X), while 112 in the Upper 5/4E group (Table XXX).
...
retouch_table1 <- retouch_table1 %>% 
  select(Quartz, Chert, Greywacke, 
        Chalcedony) %>% 
  tibble::rownames_to_column()
retouch_table1 <- dplyr::rename(retouch_table1, "Types" = "rowname")
...
retouch_table1 <- retouch_table1 %>% 
  ...
  as_tibble() %>% 
  slice(match(retouched_order, Types))
...
retouch_table2 <- retouch_table2 %>% 
  select(Quartz, Chert, Greywacke, Dolerite,
        Chalcedony) %>% 
  tibble::rownames_to_column()
retouch_table2 <- dplyr::rename(retouch_table2, "Types" = "rowname")
...
retouch_table2 <- retouch_table2 %>% 
  ...
  mutate("Dolerite (%)" = paste0(round(100 * Dolerite/sum(Dolerite), 2), "%")) %>% 
  mutate("Chalcedony (%)" = paste0(round(100 * Chalcedony/sum(Chalcedony), 2), "%")) %>% 
  select("Types", "Quartz", "Quartz (%)", "Chert", "Chert (%)", "Greywacke", "Greywacke (%)", "Dolerite", "Dolerite (%)",
         "Chalcedony", "Chalcedony (%)") %>% 
  dplyr::rename("Quartz (n)" = "Quartz", "Chert (n)" = "Chert", "Greywacke (n)" = "Greywacke", "Dolerite (n)" = "Dolerite",
         "Chalcedony (n)" = "Chalcedony") %>% 
  adorn_totals(where = "col", name = "Total") %>% 
  ...
  adorn_totals(where = "row", name = "Total", fill = "100%") %>% 
  as_tibble() %>% 
  slice(match(retouched_order2, Types))
...
retouch_table1lp <- retouch_table1lp %>% 
  select(Quartz, Chert) %>% 
  tibble::rownames_to_column()
retouch_table1lp <- dplyr::rename(retouch_table1lp, "Types" = "rowname")
...
retouch_table1lp <- retouch_table1lp %>% 
  ...
  adorn_totals(where = "row", name = "Total") %>% 
  as_tibble() %>% 
  slice(match(retouched_order3, Types))
...
# Read df
VCdb <- read_csv("index/data/ValeComprido_table.csv") #Vale comprido points and blanks db
...
VC_blanktype <- rbind(tabfreq(x = VCblanks_lp$RawMaterial, y = VCblanks_lp$BlankType),
...
VC_blanktype <- as.data.frame(VC_blanktype)
VC_blanktype <- VC_blanktype

means <- VCblanks_lp %>% 
  dplyr::summarise(Width=round(mean(WidthCM), digits = 2), Length=round(mean(LengthCM),digits = 2), Flattening=round(mean(FlatteningCM), digits = 2),
                   PlatformWidth=round(mean(PlatformWidthCM), digits = 2), PlatformThickness=round(mean(PlatformThicknessCM), digits = 2),
                   PlatformFlattening=round(mean(PlatformFlatteningCM), digits = 2), PlatformArea=round(mean(PlatformArea), digits = 2))
means <- t(means)
...
means <- means %>% 
  ...
  dplyr::rename(Mean = V1)
                      ...
                      tabmeans(x = VCblanks_lp$RawMaterial, y = VCblanks_lp$WidthCM),
                      tabmeans(x = VCblanks_lp$RawMaterial, y = VCblanks_lp$LengthCM),
                      tabmeans(x = VCblanks_lp$RawMaterial, y = VCblanks_lp$FlatteningCM),
                      tabmeans(x = VCblanks_lp$RawMaterial, y = VCblanks_lp$PlatformWidthCM),
                      tabmeans(x = VCblanks_lp$RawMaterial, y = VCblanks_lp$PlatformThicknessCM),
                      tabmeans(x = VCblanks_lp$RawMaterial, y = VCblanks_lp$PlatformFlatteningCM),
                      tabmeans(x = VCblanks_lp$RawMaterial, y = VCblanks_lp$PlatformArea))

means <- tabmeans(x = VCblanks_lp$RawMaterial, y = VCblanks_lp$LengthCM)
...
In Vale Boi (table \@ref(tab:VCvb), these blanks seem to be mostly flakes (71.4%), the rest being blades, while at Lapa do Picareiro (\@ref(tab:VClp) these blanks are all blades. For both sites, blank attributes seem relatively similar, with high frequencies of straight profiles (57.1% and 100% respectively), triangular cross sections (57.1% and 80%) and plain platforms (71.4% and 80%). Regarding shape, there seems to be a slight difference, where Vale Boi shows 57.1% of convergent shapes (the rest being parallel) while Lapa do Picareiro has a frequency of 60% parallel shapes (followed by convergent).

Regarding measurements, Vale Boi and Lapa do Picareiro seem to show fairly similar results, where the only relevantly differing values seem to be the length and the flattening. While the mean for length in chert and dolerite, at Vale Boi, seems to be 3.12 cm, at Lapa do Picareiro, the blanks show higher mean lengths (4.63 cm). As for flattening, while the ratio for Vale Boi is 2.91, Lapa do Picareiro shows a ratio of 4. This shows that Vale Comprido-like blanks from Lapa do Picareiro are flatter than those from Vale Boi, even if platform flattening means are fairly similar between sites, something already observed during the analysis, and which difficulted the attribution of a Vale Comprido designation to these blanks.


```{r VCvb, echo = FALSE}

knitr::kable(VC_blanks_tabVB, booktabs=TRUE, caption = "Vale Comprido blank attributes and measurement means from Vale Boi by raw material.") %>% 
  row_spec(0, bold = T, align = "c") %>% 
  kable_styling(latex_options = "scale_down")
```
...
knitr::kable(VC_blanktype, booktabs=TRUE, caption = "Vale Comprido-like chert blank attributes from Lapa do Picareiro") %>% 
  row_spec(0, bold = T, align = "c") %>% 
  kable_styling(latex_options = "scale_down")

knitr::kable(means, booktabs=TRUE, caption = "Vale Comprido-like chert blank measurement means from Lapa do Picareiro") %>% 
  row_spec(0, bold = T, align = "c") %>% 
  kable_styling(latex_options = "scale_down")
```
